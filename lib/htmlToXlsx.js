...
module.exports = function (reporter, definition) {
  ...
  reporter.documentStore.on('after-init', () => {
    reporter.documentStore.collection('templates').beforeInsertListeners.add(definition.name, (doc) => {
      if (doc.htmlToXlsx) {
        doc.htmlToXlsx.insertRawTables = true
      }
    })

    reporter.documentStore.collection('templates').beforeUpdateListeners.add(definition.name, (query, update) => {
      if (update.$set && update.$set.htmlToXlsx) {
        update.$set.htmlToXlsx.insertRawTables = true
      }
    })
  })
  ...
  const execute = async (request, response, legacy) => {
    ...
    if (legacy === true) {
      reporter.logger.debug('html-to-xlsx using legacy conversion', request)

      conversion = conversions[`${htmlToXlsxOptions.htmlEngine}Legacy`]
    } else {
    ...
    const stream = await conversion(response.content.toString(), {
      fontFamily: htmlToXlsxOptions.fontFamily != null && htmlToXlsxOptions.fontFamily !== '' ? htmlToXlsxOptions.fontFamily : null,
    ...
    if (!legacy && htmlToXlsxOptions.insertToXlsxTemplate === true) {
  ...
  reporter.extensionsManager.recipes.push({
    name: 'html-to-xlsx',
    execute: (request, response) => execute(request, response, true)
  })

  reporter.extensionsManager.recipes.push({
    name: 'html-to-better-xlsx',
  ...
  if (htmlEngines.chrome) {
    ...
    conversions.chromeLegacy = htmlToXlsx({
      ...Object.assign({}, reporter.options.chrome, options),
      legacy: true,
      extract: chromeEval
    })
  ...
  if (htmlEngines.phantom) {
    ...
    conversions.phantomLegacy = htmlToXlsx({
      ...options,
      legacy: true,
      extract: phantomEval
    })
